...
//definir un array con nombres (5) y de esos de ahi filtrar nombres con menos de 4 caracteres, usando solo 
let nombres = ["Pedro","Ana","Ivan","Alan","Fabricio"];
let nuevo_espacio=[];

for (let i=0; i<5; i++){
    if (nombres[i].length<4) {
        console.log(nombres[i]);
        nuevo_espacio.push(nombres[i]);
}
}
console.log(nuevo_espacio);
...
while (acum < 5) { 
    if (nombres[acum].length < 4) { 
        console.log(nombres[acum]); 
        nuevo_espacio.push(nombres[acum]); 
    } 
    acum++;
} 
console.log(nuevo_espacio);
...
function saludar(){ 
    return("Mi tarea es saludar,");
}
let resp = saludar();
console.log(resp);

function presentacion(){ 
    let nombre="Josue", ciudad= "Naranjito", edad="18";
    return `Hola, me llamo ${nombre}, soy de ${ciudad}, y tengo ${edad}`
}
let resp = saludar();
console.log(resp);
...
function presentacion(){ 
    ...
    return `los numeros pares son ${nuevo_espacio} `
}
    let resp = presentacion();
console.log(resp);
/*Lista
